...
(defclass terminal-pane (pane)
  ((pty         :initarg :pty
                :accessor terminal-pane-pty
                :documentation "PTY handle for this terminal session.")
   (screen      :initarg :screen
                :accessor terminal-pane-screen
                :documentation "Screen model for this terminal.")
   (vt-handler  :initarg :vt-handler
                :accessor terminal-pane-vt-handler
...
(defun make-terminal-pane (&key command args
                                (col 0) (row 0) (width 80) (height 24)
                                atlas)
  "Create a terminal pane running COMMAND with ARGS.
   ATLAS is needed for the VT handler's codepoint->glyph mapping."
  (let* ((screen (pcf-gl/model:make-screen :cols width :rows height))
         (pty (pcf-gl/pty:pty-fork command :cols width :rows height :args args))
         (pane (make-instance 'terminal-pane
                              :col col :row row :width width :height height
                              :pty pty :screen screen)))
    ;; Set up VT handler with callback
...
(defun make-terminal-pane-callback (pane)
  ...
  (lambda (type data)
    (case type
      ...
      (:report-cursor
       (when (pcf-gl/pty:pty-alive-p (terminal-pane-pty pane))
