...
(defmethod pane-initialize ((pane uterm-pane) atlas)
  ...
  (let* ((width (pane-width pane))
    ...
    (when command
      (let ((pty (lexter/pty:pty-fork command :cols width :rows height :args args)))
    ...
    (case type
      ...
      (:report-cursor
       (when (and (uterm-pane-pty pane)
         ...
         (lexter/pty:pty-write-string (uterm-pane-pty pane) data)))
      (otherwise nil))))
...
(defmethod pane-flush ((pane uterm-pane) grid)
  ...
  (when (uterm-pane-screen pane)
    (lexter/model:flush-to-display
     ...
     :space-glyph (lexter/model:screen-blank-glyph (uterm-pane-screen pane))
...
(defmethod pane-handle-key ((pane uterm-pane) key scancode action mods)
  ...
  (when (and (uterm-pane-pty pane)
    ...
    (let ((n (%key-to-bytes key mods (uterm-pane-write-buffer pane))))
      (unless (zerop n)
        (lexter/pty:pty-write (uterm-pane-pty pane)
  ...
  (let ((pty (uterm-pane-pty pane))
    ...
    (loop
      (let ((status (lexter/pty:pty-poll pty 0)))
        (case status
          (:readable
           (let ((n (lexter/pty:pty-read pty buf)))
             (when (> n 0)
...
(defmethod pane-resize ((pane uterm-pane) new-width new-height)
  ...
  (when (uterm-pane-screen pane)
    (lexter/model:resize-screen (uterm-pane-screen pane) new-width new-height))
  (when (uterm-pane-pty pane)
    (lexter/pty:pty-set-size (uterm-pane-pty pane) new-width new-height)))

(defmethod pane-dirty-p ((pane uterm-pane))
  "Check if terminal screen has dirty rows."
  (and (uterm-pane-screen pane)
       (lexter/model:any-row-dirty-p (uterm-pane-screen pane))))
